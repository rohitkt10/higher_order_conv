...
from src.model_zoo import chipseq_residual_bind
...
MODELTYPE = chipseq_residual_bind
BASELR=1e-4
EPOCHS=75
BATCHSIZE=128
MODEL_TYPES=['shallow', 'deep']
...
def get_callbacks(early_stopping=True, historysavepath=None, modelsavedir=None, monitor='val_AUROC'):
	"""
...
def train_model(train_data, conv_type='regular', model_type='shallow', resultdir=None,
	...
	L, A = x_train.shape[1:]
	if not extra_params:
		model = MODELTYPE.get_model(L=L, A=A, conv_type=conv_type, model_type=model_type)
	else:
		model = MODELTYPE.get_model(L=L, A=A, conv_type=conv_type, model_type=model_type, **extra_params)
...
def run_l1factor_experiment(train_data, l1_factors, resultdir, test_data=None):
	...
	for model_type in MODEL_TYPES:
		...
		results_dir = os.path.join(resultdir, model_type, 'regular_conv_results')
		if not os.path.exists(results_dir):
			os.makedirs(results_dir)
		train_model(train_data=train_data, conv_type='regular', model_type=model_type,
					resultdir=results_dir, epochs=EPOCHS, batch_size=BATCHSIZE, test_data=test_data)

		# loop over the list of l1 factors for the pairwise and nearest neighbor models 
		...
		for i, l1 in enumerate(l1_factors):
			...
			results_dir = os.path.join(resultdir, model_type, 'pairwise_conv_results', 'l1={:.0e}'.format(l1))
			if not os.path.exists(results_dir):
				os.makedirs(results_dir)
			extra_params = {'pairwise_regularizer_type':'l1','pairwise_regularizer_const':l1}
			train_model(train_data=train_data, conv_type='pairwise', model_type=model_type, 
				resultdir=results_dir, epochs=EPOCHS, batch_size=BATCHSIZE, extra_params=extra_params, test_data=test_data)

			print("")
			print("Training model with nearest neighbor convolutions with l1 factor {:.0e}".format(l1))
			results_dir = os.path.join(resultdir, model_type, 'nearest_neighbor_conv_results', 'l1={:.0e}'.format(l1))
			if not os.path.exists(results_dir):
				os.makedirs(results_dir)
			extra_params = {'nn_regularizer_type':'l1','nn_regularizer_const':l1}
			train_model(train_data=train_data, conv_type='nearest_neighbor', model_type=model_type, 
				resultdir=results_dir, epochs=EPOCHS, batch_size=BATCHSIZE, extra_params=extra_params, test_data=test_data)
...
def run_l2factor_experiment(train_data, l2_factors, resultdir, test_data=None):
	...
	for model_type in MODEL_TYPES:
		...
		for i, l2 in enumerate(l2_factors):
			...
			results_dir = os.path.join(resultdir, model_type, 'pairwise_conv_results', 'l2={:.0e}'.format(l2))
			if not os.path.exists(results_dir):
				os.makedirs(results_dir)
			...
			results_dir = os.path.join(resultdir, model_type, 'nearest_neighbor_conv_results', 'l2={:.0e}'.format(l2))
			if not os.path.exists(results_dir):
				os.makedirs(results_dir)
