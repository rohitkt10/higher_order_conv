...
from src.model_zoo import cnn_att
from src.layers import PairwiseConv1D, NearestNeighborConv1D
from src.regularizers import PairwiseKernelRegularizer, NearestNeighborKernelRegularizer
from src.layers import MultiHeadAttention, MultiHeadDense
custom_objects_dict = {
...
RESULTSDIR = os.path.join(os.getcwd(), '..', 'results', 'cnn_att_encode_chipseq', "max_pool_%d"%POOL_SIZE)
MODELTYPE = cnn_att
...
def train_model(train_data, conv_type='regular', resultdir=None,
	...
	L, A = x_train.shape[1:]
	if not extra_params:
		model = MODELTYPE.get_model(L=L, A=A, conv_type=conv_type, pool_size=POOL_SIZE)
	else:
		model = MODELTYPE.get_model(L=L, A=A, conv_type=conv_type, pool_size=POOL_SIZE, **extra_params)
	...
	if test_data:
		with tfk.utils.CustomObjectScope(custom_objects_dict):
			best_model = tfk.models.load_model(os.path.join(resultdir, 'best_model.hdf5'))
...
def run_l1factor_experiment(train_data, l1_factors, resultdir, test_data=None):
	"""
	Test different l1 factor regularizations for the 1st layer 
	pairwise convolution for a single TF. 
	"""
	...
	# loop over the list of l1 factors for the pairwise models 
	...
	for i, l1 in enumerate(l1_factors):
		...
		train_model(train_data=train_data, conv_type='pairwise',
			resultdir=results_dir, epochs=EPOCHS, batch_size=BATCHSIZE, 
			extra_params=extra_params, test_data=test_data)
		...
		train_model(train_data=train_data, conv_type='nearest_neighbor',
			resultdir=results_dir, epochs=EPOCHS, batch_size=BATCHSIZE, 
			extra_params=extra_params, test_data=test_data)

def run_l2factor_experiment(train_data, l2_factors, resultdir, test_data=None):
	"""
	Test different l2 factor regularizations for the 1st layer 
	pairwise convolution for a single TF. 
	"""
	# set up the directory for regular conv results
	# train the regular model
